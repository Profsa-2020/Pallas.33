...
<head>
     ...
     <title>Gestão em Mall - Vistoria em Lojas - Usuário</title>
</head>
<script>
$(function() {
     $("#ace").mask("999.999");
...
<?php
     ...
     if (isset($_SERVER['HTTP_REFERER']) == true) {
          if (limpa_pro($_SESSION['wrknompro']) != limpa_pro($_SERVER['HTTP_REFERER'])) {
               ...
               $ret = gravar_log(1,"Entrada na página de manutenção de usuário do sistema Pallas.33 - Gestão em Mall");  
          }
     }
     ...
     $sho = (isset($_REQUEST['sho']) == false ? 0  : $_REQUEST['sho']);
    ...
    if ($_SESSION['wrkopereg'] >= 2) {
          if (isset($_REQUEST['salvar']) == false) { 
              ...
              $ret = ler_usuario($cha, $nom, $sta, $tip, $sen, $ema, $tel, $cel, $val, $ace, $sho); 
          }
      }
      if (isset($_REQUEST['salvar']) == true) {
          ...
          if ($_SESSION['wrkopereg'] == 1) {
              ...
              if ($ret == 0) {
                  ...
                  $ret = gravar_log(10,"Inclusão de novo usuário: " . $nom);
                  $sen = ''; $nom = ''; $ema = ''; $sta = ''; $tip = ''; $tel = ''; $cel = ''; $val = ''; $ace = ''; $sho = 0;
              }
          }
          if ($_SESSION['wrkopereg'] == 2) {
              $ret = consiste_usu();
              if ($ret == 0) {
                  $ret = alterar_usu();
                  $ret = enviar_ema($_REQUEST['ema']);
                  $ret = gravar_log(10,"Alteração de usuário existente: " . $nom); $_SESSION['wrkmostel'] = 0;
                  $sen = ''; $nom = ''; $ema = ''; $sta = ''; $tip = ''; $tel = ''; $cel = ''; $val = ''; $ace = ''; $sho = 0;
          ...
          if ($_SESSION['wrkopereg'] == 3) {
              $ret = excluir_usu();
              $ret = gravar_log(10,"Exclusão de usuário existente: " . $nom); $_SESSION['wrkmostel'] = 0;
              $sen = ''; $nom = ''; $ema = ''; $sta = ''; $tip = ''; $tel = ''; $cel = ''; $val = ''; $ace = ''; $sho = 0;
              echo '<script>history.go(-' . $_SESSION['wrknumvol'] . ');</script>'; $_SESSION['wrknumvol'] = 1;
          }
      }
...
<body id="box01">
     <h1 class="cab-0">Usuário - Sistema Gestão em Mall - Vistoria em Lojas - Profsa Informática</h1>
               ...
               <div class="qua-3 container">
                    ...
                    <form class="tel-1" name="frmTelMan" action="" method="POST">
                         ...
                         <div class="row">
                              ...
                              <div class="col-md-6">
                                   <label>Razão Social do Shopping</label>
                                   <select id="sho" name="sho" class="form-control" >
                                             <?php $ret = carrega_sho($sho); ?>
                                   </select>
                              </div>
                              <div class="col-md-3">
...
function ultimo_cod() {
 ...
 function carrega_sho($sho) {
     $sta = 0;
     include "lerinformacao.inc";    
     $com = "Select idshopping, shorazao from tb_shopping where shostatus = 0 order by shorazao";
     $sql = mysqli_query($conexao, $com);
     if ($sho == 0) {
          echo '<option value="0" selected="selected">Selecione shopping desejado ...</option>';
     }
     while ($reg = mysqli_fetch_assoc($sql)) {        
          if ($reg['idshopping'] != $sho) {
               echo  '<option value ="' . $reg['idshopping'] . '">' . $reg['shorazao'] . '</option>'; 
          }else{
               echo  '<option value ="' . $reg['idshopping'] . '" selected="selected">' . $reg['shorazao'] . '</option>';
          }
     }
     return $sta;
}

 function ler_usuario(&$cha, &$nom, &$sta, &$tip, &$sen, &$ema, &$tel, &$cel, &$val, &$ace, &$sho) {
     include "lerinformacao.inc";
 ...
 function consiste_usu() {
     ...
     if($_REQUEST['tip'] <= 2 ) {
          if (trim($_REQUEST['sho']) == "0" || trim($_REQUEST['sho']) == "0") {
               echo '<script>alert("É obrigatório informar shopping para este tipo de usuário");</script>';
               return 3;
          }      
     }
     return $sta;
 ...
 function enviar_ema($ema_usu) {
     ...
     if (mysqli_num_rows($sql) > 0) {
         ...
         $tex .= '<title>Projeto Gestão em Mall - Administração de Vistorias</title>';
         $tex .= '</head>';
         $tex .= '<body>';
         $tex .= '<p align="center">';
         ...
         $tex .= '<p align="center"><font size="4" face="Verdana"><a href="http://www.inatoshopp.com/">';
         ...
         $asu = "Acesso ao sistema Gestão em Mall - Administração de Vistorias";
