...
<?php
     ...
     if ($_SESSION['wrkopereg'] >= 2) {
          if (isset($_REQUEST['salvar']) == false) { 
               ...
               $ret = ler_tipo($cha, $des, $sta); 
          }
      }
     if ($_SESSION['wrkopereg'] == 3) { 
          $bot = 'Deletar'; 
          $per = ' onclick="return confirm(\'Confirma exclusão de tipo de vistoria informado em tela ?\')" ';
     }
 ...
 if (isset($_REQUEST['salvar']) == true) {
      if ($_SESSION['wrkopereg'] == 1) {
           ...
           if ($sta == 0) {
                $ret = incluir_tip();
                $cod = ultimo_cod();
                $ret = gravar_log(1, 10,"Inclusão de novo tipo de vistoria: " . $des); 
      ...
      if ($_SESSION['wrkopereg'] == 2) {
           ...
           if ($sta == 0) {
                $ret = alterar_tip();
                $cod = ultimo_cod(); 
                $ret = gravar_log(1, 20,"Alteração de tipo de vistoria cadastrado: " . $des); 
                $des = ''; $sta = 00;  $_SESSION['wrkopereg'] = 1; $_SESSION['wrkcodreg'] = $cod;
           }
      }
      if ($_SESSION['wrkopereg'] == 3) {
           $ret = excluir_tip(); $bot = 'Salvar'; $per = '';
           $cod = ultimo_cod(); 
           $ret = gravar_log(1, 30,"Exclusão de tipo de vistoria cadastrado: " . $des); 
           $des = ''; $sta = 00;  $_SESSION['wrkopereg'] = 1; $_SESSION['wrkcodreg'] = $cod;
      }
} 
?>
...
<body>
     <h1 class="cab-0">Tipo de Vistoria - Gestão em Mall - Profsa Informática</h1>
          ...
          <div class="col-md-10">
               <div class="qua-3 container-fluid">
                    <div class="form-row lit-3">
                         <div class="col-md-11">
                              <span>Manutenção de Tipos de Vistoria</span>
                         </div>
                         <div class="col-md-1">
                              <form name="frmTelNov" action="man-tipovis.php?ope=1&cod=0" method="POST">
                              ...
                              <div class="col-md-12 text-center">
                                   <form class="tel-1" name="frmTelMan" action="" method="POST">
                                        <div class="form-row">
                                             ...
                                             <div class="col-md-7">
                                                  <label>Descrição do Tipo de Vistoria</label>
                                                  <input type="text" class="form-control" maxlength="50" id="des"
                                        ...
                                        <table class="table table-sm table-striped">
                                             <thead>
                                                  <tr>
                                                       ...
                                                       <th scope="col">Descrição do Tipo de Vistoria</th>
...
if ($_SESSION['wrkopereg'] == 1 && $_SESSION['wrkcodreg'] == $cod) {
     exit('<script>location.href = "man-tipovis.php?ope=1&cod=0"</script>');
}

function ultimo_cod() {
     $cod = 1;
     include "lerinformacao.inc";
     $sql = mysqli_query($conexao,"Select idtipo, tipdescricao from tb_tipovis order by idtipo desc Limit 1");
...
function carrega_tip() {
     $nro = 0;
     include "lerinformacao.inc";
     $com = "Select * from tb_tipovis where idtipo > 0 order by tipdescricao, idtipo";
     $sql = mysqli_query($conexao, $com);
     while ($reg = mysqli_fetch_assoc($sql)) {        
         $lin =  '<tr>';
         $lin .= '<td class="bot-3 text-center"><a href="man-tipovis.php?ope=2&cod=' . $reg['idtipo'] . '" title="Efetua alteração do registro informado na linha"><i class="bot-1a large material-icons">healing</i></a></td>';
         $lin .= '<td class="bot-3 text-center"><a href="man-tipovis.php?ope=3&cod=' . $reg['idtipo'] . '" title="Efetua exclusão do registro informado na linha"><i class="bot-1e large material-icons">delete_forever</i></a></td>';
         $lin .= '<td class="text-center">' . $reg['idtipo'] . '</td>';
         $lin .= '<td class="text-left">' . $reg['tipdescricao'] . "</td>";
         if ($reg['tipstatus'] == 0) {$lin .= "<td>" . "Normal" . "</td>";}
         if ($reg['tipstatus'] == 1) {$lin .= "<td>" . "Bloqueado" . "</td>";}
         if ($reg['tipstatus'] == 2) {$lin .= "<td>" . "Suspenso" . "</td>";}
         if ($reg['tipstatus'] == 3) {$lin .= "<td>" . "Cancelado" . "</td>";}
         if ($reg['datinc'] == null) {
               $lin .= "<td>" . '' . "</td>";
          }else{
...
function incluir_tip() {
     $ret = 0;
     include "lerinformacao.inc";
     $sql  = "insert into tb_tipovis (";
     $sql .= "tipstatus, ";
     $sql .= "tipvistoria, ";
     $sql .= "tipdescricao, ";
     $sql .= "keyinc, ";
...
function ler_tipo(&$cha, &$des, &$sta) {
     include "lerinformacao.inc";
     $sql = mysqli_query($conexao,"Select * from tb_tipovis where idtipo = " . $cha);
     if (mysqli_num_rows($sql) == 0) {
         echo '<script>alert("Código do tipo informado não cadastrada no sistema");</script>';
     }else{
         $lin = mysqli_fetch_array($sql);
         $cha = $lin['idtipo'];
         $des = $lin['tipdescricao'];
         $sta = $lin['tipstatus'];
     }
     return $cha;
 }

 function alterar_tip() {
     $ret = 0;
     include "lerinformacao.inc";
     $sql  = "update tb_tipovis set ";
     $sql .= "tipstatus = '". $_REQUEST['sta'] . "', ";
     $sql .= "tipdescricao = '". $_REQUEST['des'] . "', ";
     $sql .= "keyalt = '" . $_SESSION['wrkideusu'] . "', ";
     $sql .= "datalt = '" . date("Y/m/d H:i:s") . "' ";
     $sql .= "where idtipo = " . $_SESSION['wrkcodreg'];
     $ret = mysqli_query($conexao,$sql);
     if ($ret == false) {
         print_r($sql);
 ...
 function excluir_tip() {
     $ret = 0;
     include "lerinformacao.inc";
     $sql  = "delete from tb_tipovis where idtipo = " . $_SESSION['wrkcodreg'] ;
     $ret = mysqli_query($conexao,$sql);
     if ($ret == false) {
         print_r($sql);
