...
<head>
     ...
     <title>Gestão em Mall - Vistoria em Lojas - Login</title>
</head>
<?php
     $sta = 00;
     $ret = 00;
                         ...
                         if ($_SESSION['wrktipusu'] < 0) {
                             ...
                             if ($lem == "S") {
                               ...
                               if ($_SESSION['wrktipusu'] == 1) {
                                 $ret = gravar_log(1,"Entrada para acesso ao sistema Pallas.33 - Menu.01 - Gestão em Mall");  
                                 exit('<script>location.href = "menu05.php"</script>');
                             }else if ($_SESSION['wrktipusu'] == 2) {
                                 $ret = gravar_log(2,"Entrada para acesso ao sistema Pallas.33 - Menu.02 - Gestão em Mall");  
                                 exit('<script>location.href = "menu05.php"</script>');
                             }else if ($_SESSION['wrktipusu'] == 3) {
                                 $ret = gravar_log(3,"Entrada para acesso ao sistema Pallas.33 - Menu.04 - Gestão em Mall");  
                                 exit('<script>location.href = "menu05.php"</script>');
                             }else if ($_SESSION['wrktipusu'] == 4) {
                                    $ret = gravar_log(4,"Entrada para acesso ao sistema Pallas.33 - Menu.04 - Gestão em Mall");  
                                    exit('<script>location.href = "menu05.php"</script>');
                                }else{
                                 $_SESSION['wrkhabcon'] = '';
                                 $ret = gravar_log(5,"Entrada para acesso ao sistema Pallas.33 - Menu.05 - Gestão em Mall");  
                                 exit('<script>location.href = "menu05.php"</script>');
                             }
                         }
                     }
...
<body class="login">
     <h1 class="cab-0">Login Inicial Sistema Gestão em Mall - Vistoria de Lojas - Profsa Informática</h1>
     <div class="entrada">
          <div class="qua-1 animated bounceInDown">
               <form name="frmLogin" action="" method="POST">
                    <br /><br />
                    <div class="row">
                         <a href="http://www.gestaomall.com.br/">
                              <img src="img/logo05.png" alt="Logotipo da empresa Gestão Mall"
                                   title="Acesso ao site principal da empresa Gestão Mall" />
                         </a>
                    </div>
                    <br /><br />
                    <div class="row">
     ...
     if ($sta == 0) {
         ...
         if ($sta == 0) {
             if ($_SESSION['wrktipusu'] == 1) {
                 ...
                 $ret = gravar_log(3,"Entrada para acesso ao sistema Pallas.33 - Menu.03 - Gestão em Mall");  
                 exit('<script>location.href = "menu05.php"</script>');
             }else if ($_SESSION['wrktipusu'] == 4) {
                 $ret = gravar_log(4,"Entrada para acesso ao sistema Pallas.33 - Menu.04 - Gestão em Mall");  
                 exit('<script>location.href = "menu05.php"</script>');
